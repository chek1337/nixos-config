{ inputs, ... }:
{
  flake.modules.homeManager.yandex-browser =
    ...
    in
    {
      ...
      # NOTE: не работает. Аналогично chromium.nix — см. комментарий там.
      # Дополнительная проблема для Yandex: расширение скачивается с Chrome Web Store при первом
      # запуске браузера. Если activation отработал до запуска браузера — Vimium ещё не скачан,
      # патчить нечего, маркер пишется вхолостую. После скачивания браузер создаёт свежий settings.js
      # (непропатченный) и Sync storage со стандартными дефолтами. Приходится делать just hm повторно.
      # Даже после двух циклов (скачать → just hm → перезапустить) хинты остаются жёлтыми
      # по той же причине, что и в Chromium: insertCSS перекрывается content_script CSS.
      ...
      #         $DRY_RUN_CMD ${pkgs-unstable.python3}/bin/python3 \
