{ ... }:
{
  # Биндинги привязаны к insert и cmdline режимам — нет смысла парсить lua до
  # первого захода в один из них. Откладываем определение _G.zsh_word_kill до
  # первого `InsertEnter` или `CmdlineEnter` (once = true → срабатывает один раз).
  extraConfigLua = ''
  ...
  keymaps = [
    {
      key = "<C-W>";
      mode = "i";
      action.__raw = "function() _G.zsh_word_kill.backward_kill_word() end";
      options.desc = "z4h backward-kill-word";
    }
    {
      key = "<C-Del>";
      mode = "i";
      action.__raw = "function() _G.zsh_word_kill.kill_word() end";
      options.desc = "z4h kill-word forward";
    }
    {
      key = "<M-BS>";
      mode = "i";
      action.__raw = "function() _G.zsh_word_kill.backward_kill_zword() end";
      options.desc = "z4h backward-kill-zword (whole arg)";
    }

    {
      key = "<C-W>";
      mode = "c";
      action.__raw = "function() _G.zsh_word_kill.cmd_backward_kill_word() end";
      options.desc = "z4h backward-kill-word (cmdline)";
    }
    {
      key = "<C-Del>";
      mode = "c";
      action.__raw = "function() _G.zsh_word_kill.cmd_kill_word() end";
      options.desc = "z4h kill-word forward (cmdline)";
    }
    {
      key = "<M-BS>";
      mode = "c";
      action.__raw = "function() _G.zsh_word_kill.cmd_backward_kill_zword() end";
      options.desc = "z4h backward-kill-zword (cmdline)";
    }
  ];
}
